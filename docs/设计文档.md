...
|2|字符串增强功能|1. 支持INCR/INCRBY/DECR/DECRBY等原子操作;<br />2. 支持64位有符号整数范围;<br />3. 支持MSET/MGET批量操作|见3.1, 6.3|
|3|列表类型|1. 支持LPUSH/RPUSH/LPOP/RPOP双向操作;<br />2. 支持LRANGE范围查询;<br />3. 列表元素最多2^32-1个|见3.1, 6.4|
...
|7|过期与淘汰策略|1. 支持EXPIRE/PEXPIRE/TTL/PTTL命令;<br />2. 支持惰性删除和时间轮主动删除;<br />3. 支持7种内存淘汰策略;<br />4. 毫秒级精度|见3.1, 6.8|
|8|事务功能|1. 支持MULTI/EXEC/DISCARD事务;<br />2. 支持WATCH/UNWATCH乐观锁;<br />3. 保证原子性和隔离性;<br />4. 单个事务建议不超过100个命令|见3.1, 6.9|
|9|持久化能力|1. 支持RDB快照持久化;<br />2. 支持AOF日志持久化;<br />3. 支持AOF重写;<br />4. 支持数据恢复|见4.2, 6.10|
|10|高可用能力|1. 支持主从复制;<br />2. 支持全量和增量同步;<br />3. 支持复制积压缓冲区|见4.3, 6.11|
|11|性能要求|1. GET/SET操作P99<1ms;<br />2. 单实例QPS≥100,000;<br />3. 支持10,000并发连接;<br />4. 每个连接内存占用<10KB|见2.2, 6.1|
|12|安全性要求|1. 支持密码认证|见4.5.9, 6.12|
...
|数据结构|当前实现|说明|
|---|---|---|
|List|双向链表|使用Go实现的双向链表|
|Hash|map|使用Go原生map|
|Set|map|使用Go原生map|
|SortedSet|slice+map|切片保持排序，map提供O(1)查找|
|String|[]byte|Go字节数组|
...
**5. 数据结构层**:
   - **String**: []byte结构,带长度前缀,二进制安全
   - **List**: 双向链表实现
   - **Hash**: map实现
   - **Set**: map实现,自动去重
   - **SortedSet**: slice+map组合实现,按score排序
...
|maxmemory-policy|内存淘汰策略|noeviction|noeviction/allkeys-lru/volatile-lru/allkeys-lfu/volatile-lfu/allkeys-random/volatile-random/volatile-ttl|
...
type SortedSet struct {
    members map[string]*sortedSetMember  // member -> *sortedSetMember
    elements []*sortedSetMember           // 按 score 排序的切片
}
...
func execHSet(db *DB, args [][]byte) Reply {
    ...
    var hash *Hash
    if !exists {
        hash = &Hash{m: make(map[string][]byte)}
